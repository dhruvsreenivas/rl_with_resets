...
class Dataset(object):
    ...
    def sample_before_index(self, index: int, batch_size: int) -> Batch:
        indx = np.random.randint(0, index, size=batch_size)
        return Batch(
            observations=self.observations[indx],
            actions=self.actions[indx],
            rewards=self.rewards[indx],
    ...
    def sample_after_index(self, index: int, batch_size: int) -> Batch:
        indx = np.random.randint(index, self.size, size=batch_size)
        return Batch(
            observations=self.observations[indx],
            actions=self.actions[indx],
            rewards=self.rewards[indx],
    ...
    def sample_first_k(self, k: int) -> Batch:
        assert k <= self.size
        return Batch(
    ...
    def sample_between(self, start: int, end: int) -> Batch:
        assert (
            start >= 0 and end <= self.size
        ), f"Not in bounds. Start is {start}, end is {end}, current size is {self.size}."
